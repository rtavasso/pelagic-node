...
class InferenceEngine:
    ...
    def run(self, spectrogram: np.ndarray) -> Tuple[int, float]:
        """
        ...
        imagefolder_class = int(np.argmax(probabilities))
        confidence = float(np.max(probabilities))

        # Remap to spec order
        spec_class = self.IMAGEFOLDER_TO_SPEC[imagefolder_class]

        return spec_class, confidence
...
def run_model(engine: InferenceEngine, spectrogram: np.ndarray) -> Tuple[int, float]:
    """

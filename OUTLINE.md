# Master Engineering Specification: Virtual Edge-Node for Acoustic Anomaly Detection
**Version:** 5.2 (Golden Master)
...
1.  **Environment Setup:**
    ...
    *   Install dependencies with uv (see `requirements.txt` below for runtime vs training split).

2.  **Phase 0 (Synthetic Data):**
    *   Write `generate_dummy_data.py`. Ensure it outputs `.wav` files first, then converts to `.png`.
    *   **Unit Test:** Verify `dsp_pipeline.py` creates a 224x224 tensor from these files.

3.  **Model Training:**
    *   Write `train_model.py`. Train for 5 epochs on dummy data. Export ONNX.

4.  **Simulation Logic (The Core):**
    *   Implement `hardware_sim.py` (Battery & Input Stream).
    *   Implement `main.py` (State Machine Loop).
    *   *Critical Logic Check:* Ensure `buffer.clear()` happens on entry to TRANSMIT AND on exit from SLEEP.

5.  **Phase 1 (Real Data):**
    *   Swap dummy generator for real NOAA downloads.
    *   Retrain.
    *   Run full simulation.
...
### Runtime Dependencies (Simulation)
```text
# requirements.txt
numpy
librosa
soundfile          # For reading .wav/.flac files
onnxruntime        # ONNX model inference
```

### Training Dependencies (Model Development)
```text
# requirements-training.txt
torch
torchaudio
torchvision       # For MobileNetV2 base model
matplotlib          # For spectrogram visualization
Pillow              # For image saving
```

**Note:** Training dependencies are only needed for `generate_dummy_data.py` and `train_model.py`. The simulation runtime (`main.py`) only requires the runtime dependencies.